...
def handler(event, context):
    ...
    try:
        ...
        print(f"DEBUG: Found {len(existing_requests)} existing requests (excluding current)")
        for req in existing_requests:
            print(f"DEBUG: Added existing request - ID: {req['id']}, Title: {req['title'][:50]}...")
...
def get_existing_requests_excluding_current(current_id):
    """Read existing requests from raw JSON bucket, excluding current request and its variants"""
    try:
        ...
        # Extract base ID (remove all suffixes)
        base_current_id = current_id.split('-test')[0].split('-retry')[0].split('-final')[0].split('-fixed')[0].split('-ultimate')[0].split('-process')[0].split('-trigger')[0]
        print(f"DEBUG: Skipping current request ID: {current_id}")
        print(f"DEBUG: Base current ID: {base_current_id}")
        
        requests = []
        for obj in response['Contents'][-10:]:
            if obj['Key'].endswith('.json'):
                file_id = obj['Key'].split('/')[-1].replace('.json', '')
                base_file_id = file_id.split('-test')[0].split('-retry')[0].split('-final')[0].split('-fixed')[0].split('-ultimate')[0].split('-process')[0].split('-trigger')[0]
                
                # Skip if this is the current request or any variant of it
                if base_file_id == base_current_id:
                    print(f"DEBUG: Skipping current request ID: {file_id}")
                    continue
                ...
                try:
                    ...
                    if feature_req.get('title') and feature_req.get('description'):
                        requests.append({
                            'id': base_file_id,  # Use filename base ID, not JSON ID
