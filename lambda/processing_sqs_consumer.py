...
def handler(event, context):
    """SQS-triggered processing with FRMF extensions"""
    try:
        ...
        for record in event['Records']:
            ...
            logger.info(f"Processing SQS message for request: {request_id}")
            ...
            # Step 2: Claude Deduplication (pass filename ID)
            filename_id = key.split('/')[-1].replace('.json', '')
            dedup_result = invoke_claude_deduplication(classification_result, filename_id)
        ...
        return {
            'statusCode': 200,
            'body': {
                'message': 'SQS batch processed successfully',
...
def invoke_claude_deduplication(data, filename_id=None):
    """Invoke Claude deduplication Lambda"""
    try:
        feature_req = data.get('feature_request', {})
        # Use filename ID if provided, otherwise use data ID
        request_id = filename_id if filename_id else data.get('id', '')
        claude_payload = {
            'id': request_id,  # Pass the correct request ID
            'title': feature_req.get('title', ''),
            'description': feature_req.get('description', '')
        }
